...
def train_yolov8(yolo_path, img_size, batch, epochs, data_path, project_name, run_name, versions):
    ...
    for version in versions:
        ...
        os.system(
            f'mv {os.getcwd()}/yolov8{version}.pt {os.getcwd()}/models/yolov8{version}.pt',
        )

        # Copying the best model to models folder in current directory with specific naming convention
        os.system(
            f'cp {project_name}/{modified_run_name}/weights/best.pt {os.getcwd()}/models/yolov8{version}_best.pt',
        )
